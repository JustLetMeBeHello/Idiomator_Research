...
Trains Stage 1 (mBERT classifier) for all 15 language combos.
Run this in one Colab session; safe to rerun after a timeout.
...
Setup (run once per session):
    ...
    subprocess.run(["git", "clone",
        "https://github.com/JustLetMeBeHello/Idiomator_Research.git",
        "/content/Research_And_Training"])
    os.symlink("/content/drive/MyDrive/Idiomator_Research/models",
               "/content/Research_And_Training/models")
    os.symlink("/content/drive/MyDrive/Idiomator_Research/results",
               "/content/Research_And_Training/results")

Run:
    import subprocess, sys
    proc = subprocess.Popen(
        [sys.executable, "-u", "notebooks/colab/run_stage1.py"],
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--force", action="store_true",
                    help="Retrain even if checkpoint/output already exists. "
                         "Deletes the stale checkpoint sentinel + output dir first.")
    # Hyperparameters
...
def expected_done(args: argparse.Namespace, job: Job) -> bool:
    base = Path(args.ablation_dir) / job.combo / "stage1_mbert"
...
def main() -> None:
    ...
    for job in tqdm(jobs, desc=f"{SYSTEM} jobs", unit="combo"):
        ...
        if args.force:
            ...
            if cp.exists():
                cp.unlink()
            out_dir = Path(args.ablation_dir) / job.combo / "stage1_mbert"
            if out_dir.exists():
                shutil.rmtree(out_dir)
                tqdm.write(f"  force: deleted stale {out_dir}")

        out = str(Path(args.ablation_dir) / job.combo / "stage1_mbert")
        cmd = [
            args.python, "-u", "training/Stage_1_training.py",

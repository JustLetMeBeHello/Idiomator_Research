...
Trains BIO tagger (token-level sequence labeling baseline) for all 15 language combos.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--force", action="store_true",
                    help="Retrain even if checkpoint/output already exists. "
                         "Deletes the stale checkpoint sentinel + bio_tagger output dir first "
                         "so a forced rerun can't silently mix old and new files.")
    # Hyperparameters
    p.add_argument("--lr",       default="3.27e-5")
    p.add_argument("--epochs",   default="6")
    p.add_argument("--batch",    default="32")
    p.add_argument("--dropout",  default="0.239431179668018881")
    p.add_argument("--o_weight", default="0.104")
    return p.parse_args()
...
def expected_done(args: argparse.Namespace, job: Job) -> bool:
    base = Path(args.ablation_dir) / job.combo / "bio_tagger"
...
def main() -> None:
    ...
    for job in tqdm(jobs, desc=f"{SYSTEM} jobs", unit="combo"):
        ...
        if args.force:
            ...
            if cp.exists():
                cp.unlink()
            out_dir = Path(args.ablation_dir) / job.combo / "bio_tagger"
            if out_dir.exists():
                shutil.rmtree(out_dir)
                tqdm.write(f"  force: deleted stale {out_dir}")

        out = str(Path(args.ablation_dir) / job.combo / "bio_tagger")
        cmd = [
            args.python, "-u", "experiments/ablations/BiO_Task_mBERT_train.py",

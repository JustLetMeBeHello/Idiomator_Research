...
Trains Stage 2 (mBERT QA span extractor) for all 15 language combos.
...
def expected_done(args: argparse.Namespace, job: Job) -> bool:
    base = Path(args.ablation_dir) / job.combo / "stage2_mbert"
...
def main() -> None:
    ...
    for job in tqdm(jobs, desc=f"{SYSTEM} jobs", unit="combo"):
        ...
        if args.force:
            ...
            if cp.exists():
                cp.unlink()
            out_dir = Path(args.ablation_dir) / job.combo / "stage2_mbert"
            if out_dir.exists():
                shutil.rmtree(out_dir)
                tqdm.write(f"  force: deleted stale {out_dir}")

        out = str(Path(args.ablation_dir) / job.combo / "stage2_mbert")
        cmd = [
            args.python, "-u", "training/Stage_2_training.py",

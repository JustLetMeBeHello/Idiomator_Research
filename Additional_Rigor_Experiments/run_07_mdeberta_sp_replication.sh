#!/usr/bin/env bash
# ── Experiment 07: mDeBERTa-v3 — 2nd SentencePiece encoder ──────────────────
#
...
# amplifies QA." This experiment adds encoder #4 — microsoft/mdeberta-v3-base,
# a SECOND SentencePiece encoder — to decide between:
#
#     mDeBERTa shows the large QA gain too  → "SentencePiece amplifies QA" (Main)
#     mDeBERTa does NOT                     → XLM-R-specific; claim narrows
#
# Same pipeline, same hyperparameters as the matrix, so the mDeBERTa cells drop
# straight into the existing QA−BIO gap table next to XLM-R / mBERT / MuRIL.
#
# TWO PROOFS (the notebook runs the cheap one first):
#   (1) MECHANISM, no GPU — tokenizer reachability. SentencePiece pushes gold
#       spans off word boundaries → high "subtok%" (QA-only-reachable). If
#       mDeBERTa's SP tokenizer patterns with XLM-R (not mBERT/MuRIL), that
#       alone predicts QA>BIO for SentencePiece. Runs in seconds, see notebook.
#   (2) EMPIRICAL, this script — train QA (joint) and BIO under mDeBERTa across
#       3 seeds and measure the QA−BIO exact gap per language.
#
# COMPATIBILITY GATE (hard). Train_Join.py and Ablations/BiO_Task_mBERT_train.py
# pass token_type_ids unconditionally into the encoder. mDeBERTa-v3 has
# type_vocab_size=0 (no token_type embeddings) and a SentencePiece tokenizer;
# all-zero token_type_ids SHOULD be ignored, but this has never been run. Do
# NOT launch the 6 full runs blind — run `--dry-run` first (1 epoch English,
# joint+bio) and confirm both produce metrics.json + test_predictions.jsonl.
#
# HYPERPARAMETERS. Identical to the flip matrix:
#   Joint (QA): epochs 7, bs 32, lr 1e-5 (XLM-R/SP large-model practice),
#               cls_loss_weight 0.3, span_loss_weight 1.9
#   BIO:        epochs 6, bs 32, lr 3.27e-5 (System G default, untuned for SP —
...
#   Dry-run (compat gate):  bash Additional_Rigor_Experiments/run_07_mdeberta_sp_replication.sh --dry-run
#   Full matrix:            bash Additional_Rigor_Experiments/run_07_mdeberta_sp_replication.sh
# ──────────────────────────────────────────────────────────────────────────────
...
JOINT="Train_Join.py"
BIO="Ablations/BiO_Task_mBERT_train.py"
...
run_bio () {    # $1=outdir  $2=epochs  $3=seed
    python -u "$BIO" \
        --model_name "$MODEL" --output_dir "$1" \
        --langs $LANGS --test_langs $TEST_LANGS \
        --epochs "$2" --batch_size 32 --lr 3.27e-5 --o_weight 0.104 --seed "$3" \
        2>&1 | tee -a "$1/console.log"
}
...
if [[ "$DRY_RUN" == "1" ]]; then
    echo "═══ DRY-RUN — mDeBERTa compatibility gate (1 epoch, English only) ═══"
    ...
    echo
    echo "✓ DRY-RUN PASSED — mDeBERTa is compatible with both trainers."
    echo "  Now launch the full matrix (no --dry-run). Clean the dry dirs first:"
    echo "    rm -rf $DJ $DB"
    exit 0
fi

echo "═══ FULL MATRIX — mDeBERTa-v3 QA vs BIO × seeds {42,123,7} ═══"
for SEED in 42 123 7; do
    JD="$OUT_BASE/mdeberta_joint_s${SEED}"
    BD="$OUT_BASE/mdeberta_bio_s${SEED}"
    mkdir -p "$JD" "$BD"

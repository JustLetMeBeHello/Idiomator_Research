...
python training/Train_Join.py \
    ...
    --seed 42
# NOTE: cls_loss_weight raised 0.3→0.5→1.0 for MuRIL. At 0.3 and 0.5 the cls
# head stays stuck at random-coin-flip (cls loss flat at ~0.693 all 7 epochs)
# because span gradient dominates. Equal-weight (1.0) forces cls to compete.
# All other HP identical to System E mBERT.
# Paper footnote: "MuRIL required cls_loss_weight=1.0 for joint training convergence."

echo
echo "── Experiment 03 complete ─────────────────────────────────────────────"
echo "Compare MuRIL Joint vs System E mBERT baselines (computed live, never hardcoded):"
python experiments/rigor/mbert_baselines.py --system E || \

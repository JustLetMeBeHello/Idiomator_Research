...
| `run_13_tost_equivalence.py` | **N1** — TOST equivalence test for C1 (replaces "CI contains reference"); reuses run_08 evaluate_encoder; default Δ=0.02; `--reference wp|zero` | RAN 2026-06-21, both batch-16 and batch-32 (E1) RemBERT configs — batch-16 passes marginally (p=0.045), batch-32 (deconfounded) FAILS equivalence (p=0.316). Threatens locked C1 verdict, see `memory/key_numbers.md` |
